...
// An empty template keeps the built-in layout, byte for byte, including the
// unique leaf uniqueLeaf asks for. A template supersedes uniqueLeaf, because it
// names every segment of the path including the leaf; write {repo}-{slot} for
// the same effect. Recycled slots never reach this function: they are handed
// back at the path recorded in state, so a template never moves a worktree that
// exists.
func resolveWorktreePath(repoRoot, poolDir, slot, template string, uniqueLeaf bool) (string, error) {
	...
	if template == "" {
		...
		return filepath.Join(poolDir, slot, leaf), nil
	}
