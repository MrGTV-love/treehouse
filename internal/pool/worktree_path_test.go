...
func TestResolveWorktreePath_EmptyTemplateKeepsBuiltInLayout(t *testing.T) {
	...
	got, err := resolveWorktreePath(repoRoot, poolDir, "1", "", false)
	if err != nil {
...
// TestResolveWorktreePath_UniqueLeafAppliesToTheBuiltInLayout keeps unique_leaf
// working through the resolver that replaced the layout it used to name.
func TestResolveWorktreePath_UniqueLeafAppliesToTheBuiltInLayout(t *testing.T) {
	repoRoot, poolDir := synthPaths(t)

	got, err := resolveWorktreePath(repoRoot, poolDir, "1", "", true)
	if err != nil {
		t.Fatalf("resolveWorktreePath failed: %v", err)
	}
	if want := filepath.Join(poolDir, "1", "myrepo-1"); got != want {
		t.Errorf("resolved %q, want the unique leaf %q", got, want)
	}
}

// TestResolveWorktreePath_TemplateSupersedesUniqueLeaf pins the composition rule
// between the two placement options: a template names every segment including
// the leaf, so unique_leaf cannot also rename it. acquire warns when both are
// set; here the point is that the template alone decides the path.
func TestResolveWorktreePath_TemplateSupersedesUniqueLeaf(t *testing.T) {
	repoRoot, poolDir := synthPaths(t)
	template := "{pool}/{slot}/{repo}"

	got, err := resolveWorktreePath(repoRoot, poolDir, "1", template, true)
	if err != nil {
		t.Fatalf("resolveWorktreePath(%q, false) failed: %v", template, err)
	}
	if want := filepath.Join(poolDir, "1", "myrepo"); got != want {
		t.Errorf("resolved %q, want the template's own leaf %q", got, want)
	}

	// The template spells the unique leaf itself when that is what you want.
	got, err = resolveWorktreePath(repoRoot, poolDir, "1", "{pool}/{slot}/{repo}-{slot}", false)
	if err != nil {
		t.Fatal(err)
	}
	if want := filepath.Join(poolDir, "1", "myrepo-1"); got != want {
...
func TestResolveWorktreePath_Templates(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveWorktreePath(repoRoot, poolDir, "2", tc.template, false)
			if err != nil {
				t.Fatalf("resolveWorktreePath(%q, false) failed: %v", tc.template, err)
			}
			if got != tc.want {
...
func TestResolveWorktreePath_RejectedTemplates(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveWorktreePath(repoRoot, poolDir, "1", tc.template, false)
			if err == nil {
	...
	if _, err := resolveWorktreePath(repoRoot, inProjectPool, "1", "{pool}/{slot}/{repo}-{slot}", false); err != nil {
...
func TestResolveWorktreePath_TwoSlotsNeverShareADirectory(t *testing.T) {
	...
	first, firstErr := resolveWorktreePath(repoRoot, poolDir, "1", template, false)
	second, secondErr := resolveWorktreePath(repoRoot, poolDir, "2", template, false)
...
func TestResolveWorktreePath_SiblingRepositoriesNeverShareASlotDirectory(t *testing.T) {
	...
	firstPath, firstErr := resolveWorktreePath(first, poolDir, "1", unscoped, false)
	secondPath, secondErr := resolveWorktreePath(second, poolDir, "1", unscoped, false)
	...
	firstPath, err := resolveWorktreePath(first, poolDir, "1", scoped, false)
	if err != nil {
		t.Fatalf("expected %q to stay accepted: %v", scoped, err)
	}
	secondPath, err = resolveWorktreePath(second, poolDir, "1", scoped, false)
	if err != nil {
		t.Fatalf("expected %q to stay accepted: %v", scoped, err)
	}
...
func TestResolveWorktreePath_RejectsSwallowingTheRepositoryOrPool(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveWorktreePath(tc.repoRoot, tc.poolDir, "1", tc.template, false)
			if err == nil {
				t.Fatalf("expected %q to be rejected, resolved to %q", tc.template, got)
			}
			if !strings.Contains(err.Error(), tc.wants) {
...
func TestResolveWorktreePath_SymlinksDoNotBypassPlacement(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := resolveWorktreePath(repoRoot, poolDir, "1", template, false)
			if err == nil {
	...
	if _, err := resolveWorktreePath(repoRoot, spelled, "1", "{pool}/{slot}/{repo}-{slot}", false); err != nil {
...
// TestDropStaleJJSeedAuthentication_WarnsWhenRemovalFails pins the diagnostic
// half of the best-effort cleanup. The failure must not fail the caller, whose
// worktree is already deleted, and must not be silent either: the operator is
// the one who can act on the path it names.
func TestDropStaleJJSeedAuthentication_WarnsWhenRemovalFails(t *testing.T) {
	// The worktree is still on disk, so the removal refuses to unlink an
	// authentication that may still be in use.
	authPath, entry, poolDir, worktree := seededJJSlotOutsideThePool(t)
	warned := captureStaleJJSeedAuthenticationWarnings(t)

	dropStaleJJSeedAuthentication(poolDir, entry)

	if len(*warned) != 1 {
		t.Fatalf("warnings = %v, want exactly one", *warned)
	}
	if !strings.Contains((*warned)[0], worktree) {
		t.Errorf("warning %q does not name the worktree %q", (*warned)[0], worktree)
	}
	if _, err := os.Lstat(authPath); err != nil {
		t.Errorf("expected a live workspace's authentication to be left alone: %v", err)
	}
}

// TestDropStaleJJSeedAuthentication_SilentWhenRemovalSucceeds keeps the warning
// rare enough to be worth reading.
func TestDropStaleJJSeedAuthentication_SilentWhenRemovalSucceeds(t *testing.T) {
	authPath, entry, poolDir, worktree := seededJJSlotOutsideThePool(t)
	...
	if _, err := os.Lstat(authPath); !os.IsNotExist(err) {
		t.Errorf("expected the stale authentication to be removed, stat err: %v", err)
	}
}

func captureStaleJJSeedAuthenticationWarnings(t *testing.T) *[]string {
	t.Helper()
	var warned []string
	original := warnStaleJJSeedAuthentication
	warnStaleJJSeedAuthentication = func(path string, err error) {
		warned = append(warned, fmt.Sprintf("%s: %v", path, err))
	}
	t.Cleanup(func() { warnStaleJJSeedAuthentication = original })
	return &warned
}
